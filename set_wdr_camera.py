...
class CameraAdjustParameter:
    ...
    def __init__(self):
        ...
        self.camera_parameters = ['brightness', 'contrast', 'saturation', 'hue', 'gamma', 'white_balance_temperature_auto', 'white_balance_temperature', 'sharpness', 'backlight_compensation', 'exposure_auto', 'exposure_absolute']
        self.default_value = ['64', '60', '0', '180', '300', '0 = off, 1 = on', 'min = 2800, default = 4600', '65', '1', '0 = off, 1 = on', '166']
        self.brightness = 64  # -64~64
        self.contrast = 60
        self.saturation = 0
        self.hue = 180         # -180~180
        self.gamma = 300
        self.gain = 0
    ...
    def set_init_controlbar_parameter_value(self, camera_parameter_values):
        ...
        for data in camera_parameter_values:
            ...
            if str(data[:data.find(":")]) == self.camera_parameters[3]:
                bar_value = 180 + bar_value
            if str(data[:data.find(":")]) == self.camera_parameters[9]:
                if bar_value == 3:
                    bar_value = 1
                else:
                    bar_value = 0
    ...
    def get_camera_new_parameter(self):
        ...
        for item in self.camera_parameters:
            ...
            #     save_parameters.append(item + '=' + str(self.gain))
                ...
                if self.exposure_absolute < 50:
                    save_parameters.append(item + '=50')
                else:
    ...
    def run_camera(self, camera_number):
        ...
        cv2.createTrackbar('White Balance Temperature Auto\n' + self.default_value[5], 'Control', 0 , 1, self.nothing)
        cv2.createTrackbar('White Balance Temperature\n' + self.default_value[6], 'Control', 2800, 6500, self.nothing)
        cv2.createTrackbar('Sharpness\n' + self.default_value[7], 'Control', 0, 100, self.nothing)
        cv2.createTrackbar('Backlight Compensation\n' + self.default_value[8], 'Control', 0, 2, self.nothing)
        cv2.createTrackbar('Exposure Auto\n' + self.default_value[9], 'Control', 0 , 1, self.nothing)
        cv2.createTrackbar('Exposure Absolute\n' + self.default_value[10], 'Control', 50, 10000, self.nothing)
